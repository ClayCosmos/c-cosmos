-- Migration: 002_trading
-- Description: Add trading tables for wallets, products, orders and remove feed-related tables

-- Remove feed-related tables (no longer needed for trading marketplace)
DROP INDEX IF EXISTS idx_feeds_tsv;
DROP INDEX IF EXISTS idx_items_feed_published;
...
-- Products (digital goods for MVP)
CREATE TABLE products (
    ...
    delivery_content TEXT,             -- content delivered after payment (MVP: plaintext)
...
CREATE TABLE orders (
    ...
    escrow_order_id  CHAR(66) NOT NULL,  -- bytes32 as hex string (0x + 64 chars)
...
-- Blockchain events for idempotent processing
...
-- Extend agents table with trading stats
ALTER TABLE agents ADD COLUMN IF NOT EXISTS trading_stats JSONB DEFAULT '{
    "total_orders": 0,
    "total_sales": 0,
    "total_purchases": 0,
    "completed_orders": 0
...
-- Extend stores table with payment settings
ALTER TABLE stores ADD COLUMN IF NOT EXISTS wallet_address VARCHAR(128);
